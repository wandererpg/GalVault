---
entity_id: "vndb:person:s38816"
type: "person"
display_title: "Suffolk"
title_status: "unresolved"
source_ids:
  vndb: "s38816"
source_urls:
  vndb: "https://vndb.org/s38816"
---

# Suffolk

## 会社与职责
<!-- AUTO:BEGIN duty --><!-- AUTO:END duty -->

## 创作与出演
<!-- AUTO:BEGIN role -->- [[マブラヴ アンリミテッド ザ・デイアフター]]（editor）
- [[マブラヴ オルタネイティヴ クロニクルズ 告白]]（staff）
- [[マブラヴ オルタネイティヴ トータル・イクリプス 帝都燃ゆ]]（editor）
- [[マブラヴ オルタネイティヴ トータル・イクリプス]]（editor）<!-- AUTO:END role -->

## 角色
<!-- AUTO:BEGIN character --><!-- AUTO:END character -->

## 声优
<!-- AUTO:BEGIN voice --><!-- AUTO:END voice -->

## 发行版
<!-- AUTO:BEGIN release --><!-- AUTO:END release -->

## 动画化
<!-- AUTO:BEGIN adaptation --><!-- AUTO:END adaptation -->

## 关系
<!-- AUTO:BEGIN relation --><!-- AUTO:END relation -->

## 标签
<!-- AUTO:BEGIN tag --><!-- AUTO:END tag -->

## 来源
<!-- AUTO:BEGIN provenance -->vndb:s38816<!-- AUTO:END provenance -->
